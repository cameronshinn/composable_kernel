...
struct GridwiseElementwise
{
                               ...
                               const ElementwiseOperation& elementwise_op)
    {
        ...
        using DstDimAccessOrder =
            std::conditional_t<InOutSameVectorDim, Sequence<1, 0>, Sequence<0, 1>>;
        using SrcVectorDim = Number<0>;
        using DstVectorDim = std::conditional_t<InOutSameVectorDim, Number<0>, Number<1>>;

        auto global_to_global_transfer = ThreadGroupTensorSliceTransfer_v4r1<
            ThisThreadBlock,
            ...
            false,
            false,
            I1>{in_grid_desc_tuple.At(I0),
                thread_grid_offset,
                elementwise_op,
                out_grid_desc_tuple.At(I0),
                thread_grid_offset,
                elementwise_op};
        global_to_global_transfer.RunRead(
            in_grid_desc_tuple.At(I0), in_global_buf_tuple.At(I0), I0);
        global_to_global_transfer.RunWrite(
            out_grid_desc_tuple.At(I0), out_global_buf_tuple.At(I0), I0);
    }
};
